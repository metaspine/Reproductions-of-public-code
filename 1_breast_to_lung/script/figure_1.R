...
##########################################################################################
## objects
load('../data/fig1/seu_imm_fin_2.Robj')
load('../data/fig1/immune_markers.Robj')
load('../data/fig1/jsd_imm.Robj')
load('../data/fig1/anno_freq_summary_imm.Robj')
load('../data/fig1/prop_test_imm.Robj')
...
table(seu_imm_fin_2$celltype_fig1)
pdf("../images/Figure 1A-Dimplot.pdf", width = 12, height = 12)
DimPlot(seu_imm_fin_2, group.by='celltype_fig1', label = T,
        cols=c('skyblue3','indianred1','black','darkred','red','turquoise4','turquoise','dodgerblue','lightsalmon','darkorchid4','royalblue3','navy','palegreen3','maroon','turquoise3','grey','pink3'), raster=F, pt.size = 0.01) + 
...
g <- DotPlot(seu_imm_fin_2, features=unique(immune_markers), 
             cols='RdGy', dot.scale = 3) + 
  theme(axis.title.x = element_blank(), axis.title.y = element_blank(), axis.line = element_line(size=1.25), axis.ticks = element_line(size=1.25), axis.ticks.length=unit(.2, "cm")) + RotatedAxis()
  
g$data$id <- factor(g$data$id, levels=rev(levels(g$data$id)))
pdf("../images/Figure 1B-Dotplot.pdf", width = 20, height = 6)
g
dev.off()
## Fig. 1C: Timepoint JSD heatmap 
seu_imm_fin_2 = readRDS("../../../livermetastasis/data/240410/merge_2_seurat_ouput_from_5_broad_re_annotation.rds")
source("GloScope_function.R") # from R package: GloScope
jsd_imm <- jsd_comp(seu = seu_imm_fin_2, reduction = "umap_harmony",condition = 'orig.ident',nPC = 2)
library(ComplexHeatmap)
pdf("../images/Figure 1C-Heatmap.pdf", width = 12, height = 12)
Heatmap(jsd_imm, col = brewer.pal(10,'RdGy'), 
        show_column_names = F, 
...
anno_freq <- reshape2::melt(anno_freq)
...
for (i in 1:nrow(anno_freq_summary_imm)) {
  ...
  anno_freq_summary_imm$se[i] <- std.error(anno_freq$value[which(anno_freq$Var2 == ct & anno_freq$clade == cl)])
}
anno_freq_summary_imm$clade <- factor(anno_freq_summary_imm$clade, levels=c('wt','early','mid','late'))
seu_imm_fin_2@meta.data$celltype_fig1 |> table()
seu_imm_fin_2@meta.data$celltype_fig1 = factor(seu_imm_fin_2@meta.data$celltype_fig1,levels = unique(seu_imm_fin_2@meta.data$celltype_fig1))
anno_freq_summary_imm$celltype <- factor(anno_freq_summary_imm$celltype, levels=levels(seu_imm_fin_2@meta.data$celltype_fig1))
pdf("../images/Figure 1D-Barplot.pdf", width = 15, height = 8)
...
propeller_list <- list()
source("propeller_function.R") # from the package speckle;
...
prop_test_imm <- reshape2::acast(prop_test_imm, comparison~celltype, value.var="pval_bin")
